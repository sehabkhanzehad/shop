...
class PaymentMethodController extends Controller
{
    public function index(){
        ...
        return view('admin.payment_method', compact('paypal','stripe','razorpay','bank','paystackAndMollie','flutterwave','instamojo','countires','currencies','setting','paymongo','sslcommerz', 'bkash'));
    ...
    public function updateBkash(Request $request)
    {
        $rules = [
            ...
            'base_url' => $request->status ? 'required' : '',
            'username' => $request->status ? 'required' : '',
            'password' => $request->status ? 'required' : '',
            'app_key' => $request->status ? 'required' : '',
            'app_secret' => $request->status ? 'required' : '',
        ];
        // $customMessages = [
        //     'store_id.required' => trans('admin_validation.Store id is required'),
        //     'store_password.required' => trans('admin_validation.Store password is required'),
        ...
        $bkash = BkashPayment::first();
        $bkash->mode = $request->account_mode;
        $bkash->base_url = $request->base_url;
        $bkash->username = $request->username;
        $bkash->password = $request->password;
        $bkash->app_key = $request->app_key;
        $bkash->app_secret = $request->app_secret;
        $bkash->status = $request->status ? 1 : 0;
        $bkash->save();
