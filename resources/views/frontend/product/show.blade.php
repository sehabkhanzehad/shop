 ...
 src="{{asset('uploads/custom-images/'.$product->thumb_image)}}" alt="Image 1">
              ...
              @foreach($product->gallery as $key => $img_gals)
               ...
               <div class="thumbnail-container">
                  <br/>
                  <img class="thumbnail img-thumbnail" src="{{asset('uploads/custom-images/'.$product->thumb_image)}}" alt="" class="" onclick="changeImage('{{asset('uploads/custom-images/'.$product->thumb_image)}}')">
            ...
            <div class=" p-2 ps-3">
               <div class="product-content">
                  ...
                  <div class="payment-options">
                        ...
                        {{$product->price}}</del> tk (<span id="dis_amount">{{$product->price - $product->offer_price}}</span> tk discount)
                        @endif
                     	</div>
                     	...
                     	@else
                     	<input type="hidden" id="retrieve_discount" value="{{ $product->price-$product->offer_price }}">
                     	@endif
                        </div>

                  </div>
                  @else
                  <h5 class="semi" style="margin-left:0%; padding:0px">Price : <span class="current-price-product">{{$product->price}}</span> tk</h5>
                  @endif
         ...
         <div class="tab-content bg-white p-lg-4 p-3" id="myTabsContent" style="width: 100%; padding: 0px">
            ...
            <div class="tab-pane fade" id="tab4">
               ...
               <form action="{{ route('front.product.product-reviews.store') }}" method="POST">
                  ...
                  <button type="submit" class="btn" style="margin-top: 15px;background: #0f134f;color: #ffffff;">Submit Review</button>
               </form>
               @else
    ...
    padding-top: 11px;">
       <h3 class="text-center">Related Products</h3>
            <div class="row">
                ...
                <div class="product_content " style="border-top:3px solid #EDEDEF; margin-top: 10%;padding: 5px 5px;
    background-color: #fff;text-align: center;">
                    <h4 class="ps-1" style="height: 35px;">
                        <a href="{{ route('front.product.show', [ $product->id ] ) }}" class="font-14" style="font-size:14px;color: #000;
    font-weight: 700;"> {{ \Illuminate\Support\Str::limit($product->name, 35)}}</a>
                    </h4>
