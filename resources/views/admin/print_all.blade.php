...
<body>
   ...
   @foreach($items as $item)
    <div class="cs-container">
        <div class="cs-invoice cs-style1" style="padding-top: 10px !important;">
                    ...
                    <div class="row" >
                        ...
                        <tbody>
                          <tr>
                            <td>
                                <address>
                              <strong>{{__('Shipping Info')}} :</strong><br>
                          ...
                          <strong>{{__('Order Info')}}:</strong><br>
                    ...
                    <div class="table-responsive ">
                      <table class="table table-striped table-hover table-md table-bordered" style="border-bottom: 1px solid #F9EDF7;">
                        <tr style="background: #F9EDF7;">
                          <th width="5%" style="line-height: 0.1em">#</th>
                          <th width="40%" style="line-height: 0.1em">{{__('admin.Product')}}</th>
                          <th width="20%" style="line-height: 0.1em">{{__('admin.Variant')}}</th>
                          @if ($setting->enable_multivendor == 1)
                          <th width="10%" style="line-height: 0.1em">{{__('admin.Shop Name')}}</th>
                          @endif
                          <th width="15%" style="line-height: 0.1em" class="text-center">{{__('Price')}}</th>
                          <th width="10%" style="line-height: 0.1em" class="text-center">{{__('admin.Quantity')}}</th>
                          <th width="10%" style="line-height: 0.1em" class="text-right">{{__('admin.Total')}}</th>
                        </tr>
                        @php
                            $subTotal = 0;
                        $total_discount = 0;
                        ...
                        @foreach ($item->orderProducts as $index => $orderProduct)
                            ...
                            @endphp
                            <tr>
                                <td style="line-height: 0.5rem;">{{ ++$index }}</td>
                                <td style="line-height: 0.5rem;"><a href="" style="text-decoration: none;color: #000000;">{{ $orderProduct->product_name }}</a></td>
                                ...
                                @endif
                                <td class="text-center" style="line-height: 0.5rem;">{{ $setting->currency_icon }}{{ $orderProduct->product->price }}</td>
                                <td class="text-center" style="line-height: 0.5rem;">{{ $orderProduct->qty }}</td>
                                @php
                                    $total = ($orderProduct->product->price * $orderProduct->qty);
                                    $total_discount += $orderProduct->total_discount;
                                    $subTotal+=$total;
                                @endphp
                                <td class="text-right" style="line-height: 0.5rem;">{{ $setting->currency_icon }}{{ $total }}</td>
                            </tr>
                            @php
                                $totalVariant = 0;
                            @endphp
                            ...
                            <table>
                                <tbody>
                                   @php
									...
									<tr>	                                  
                                        <td class="cs-width_3 cs-border_top_0 cs-bold cs-f16 cs-primary_color cs-text_right" style="padding: 1px 15px !important;">Sub Total : {{ $setting->currency_icon }}{{ round($subTotal, 2) }}</td> 
                                  </tr>
                                  <tr>
                                      <td class="cs-width_3 cs-border_top_0 cs-bold cs-f16 cs-primary_color cs-text_right" style="padding: 1px 15px !important;font-style: italic;">Discount : {{ $setting->currency_icon }}{{ round($total_discount, 2) }}</td> 
                                  </tr>
                                  <tr>
                                    <td class="cs-width_3 cs-border_top_0 cs-bold cs-f16 cs-primary_color cs-text_right" style="padding: 1px 15px !important;font-style: italic;">Shipping :{{ $setting->currency_icon }}{{ round($item->shipping_cost, 2) }}</td>
                                      	
                                    </tr>
                                  
                                  <tr>
                                    <td class="cs-width_3 cs-border_top_0 cs-bold cs-f16 cs-primary_color cs-text_right" style="padding: 1px 15px !important;">In Total :{{ round($sub_total, 2) }}</td>
